...
app.config['UPLOAD_FOLDER'] = '/home/freya/twitter-codechella/flask-tweeeter/flaskapp/static/profile_pics'
ALLOWED_EXTENSIONS = set(['png', 'jpg', 'jpeg', 'JPG', 'PNG', 'mp4'])
...
class Post(db.Model):
    ...
    media = db.Column(db.String(20), default='default.jpg')
    media_format = db.Column(db.String(20), default='image')
    summary = db.Column(db.String(40), default='Summary ALT')
...
def new_post():
    ...
    if request.method == 'POST' and form.validate():
        ...
        # Make post object
        post = Post(content=content,media=media, media_format= media_format, author=current_user())
...
def update_photo():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            flash(
                'Succesfully changed profile picture to {filename}', 'success')
...
def follow(id):
    ...
    if user_following == user_followed:
        ...
        flash('Followed {user_followed.username}', 'success')
        return redirect(url_for('home'))
...
def unfollow(id):
    ...
    if user_unfollowing == user_unfollowed:
        ...
        flash('Unfollowed {user_unfollowed.username}', 'warning')
        return redirect(url_for('home'))
...
def new_comment(post_id):
    ...
    if request.method == 'POST' and form.validate():
        ...
        # Get form content
        content = '@{commented_post.author.username}  ' + form.content.data
        ...
        flash(
            "You have replied to {commented_post.author.username}'s tweeet", 'success')
        return redirect(url_for('home'))

    return render_template('new_post.html', form=form, title="Comment to @{commented_post.author.username}'s tweeet:")
